...
# added traps for INT and QUIT using interrupt and quit functions
trap interrupt INT
trap quit QUIT
...
# put countdown in a loop that restarts if INT is detected, resetting the timer to whatever maximum initially entered in arguments
while [ $sleepCount -gt 0 ]; do
  restart=0
