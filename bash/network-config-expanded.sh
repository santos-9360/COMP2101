...
#####
# uses ip a command to list network information, uses awk to find the interface entries in the list.
# replaces colons in interface entries with nothing and assigns column 2 (the interface names) to variable.
interfaces=$(ip a | awk '/[0-9]: /{gsub(/:/,"");print $2}')
# for loop that checks every interface name and puts them through an if statement based off how many interface names there are
for int in $interfaces;do
# if the interfacce name is lo (loopback interface) then jump back to the start of the for loop on the next iteration (skipping loopback)
  if [ $int == "lo" ]; then
    continue
  fi
